...
int main( int argc, char *argv[] )
{
    ...
    while(1) 
    {
        ...
        {
            //printf( 1, "%d\n", match(record, user_name, user_password) );
            //contains( record, user_name) && contains( record, user_password )
            if ( match(record, user_name, user_password) )
            {
                printf( 1, "Log in successfull :D\n"); 
                //
                pid = fork();
                if(pid < 0){
        ...
        printf( 1, "Log in failed, wrong username or password  D:\n");
        close(fd);
    }
    exit();
}